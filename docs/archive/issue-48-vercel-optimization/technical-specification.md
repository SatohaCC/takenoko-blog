...
> [!NOTE]
> **アーカイブ文書（Issue #48 / 対応完了）**。これは実装前に作成した設計仕様です。実際に行われた変更の確定記録は [walkthrough.md](./walkthrough.md) を参照してください（仕様策定後にスコープが広がっており、`§5 Affected Files` は実装ファイルの一部のみを列挙しています）。
